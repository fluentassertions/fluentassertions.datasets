...
public class ApiApproval
{
    ...
    public Task ApproveApi(string frameworkVersion)
    {
        ...
        var assemblyFile = Path.GetFullPath(
            Path.Combine(
                GetSourceDirectory(),
                Path.Combine("..", "..", "Src", "FluentAssertions.DataSets", "bin", configurationName, frameworkVersion,
                    "FluentAssertions.DataSets.dll")));
    ...
    private class TargetFrameworksTheoryData : TheoryData<string>
    {
        public TargetFrameworksTheoryData()
        {
            var csproj = Path.Combine(GetSourceDirectory(),
                Path.Combine("..", "..", "Src", "FluentAssertions.DataSets", "FluentAssertions.DataSets.csproj"));
